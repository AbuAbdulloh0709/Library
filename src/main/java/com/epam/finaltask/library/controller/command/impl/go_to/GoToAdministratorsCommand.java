...
public class GoToAdministratorsCommand implements Command {
    ...
    public Router execute(HttpServletRequest request) {
        try {
            List<User> administrators = userService.findUsers(UserRole.ADMIN);
            request.setAttribute(ADMINISTRATORS, administrators);
        } catch (ServiceException e) {
            LOGGER.error("Error has occurred while redirecting to administrators page: " + e);
